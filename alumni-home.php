...
$event_query = "SELECT * FROM events WHERE schedule >= CURDATE() ORDER BY schedule ASC LIMIT 5";
        ...
        <?php if($event_result->num_rows > 0): ?>
          <?php while($event = $event_result->fetch_assoc()): ?>
            <div class="event-card">
              <div class="event-img">
                <?php 
                  $banner = !empty($event['banner']) ? $event['banner'] : 'plpasigg.jpg';
                  if(!file_exists('images/'.$banner)){
                    $banner = 'plpasigg.jpg';
                  }
                ?>
                <img src="images/<?php echo $banner; ?>" alt="<?php echo htmlspecialchars($event['title']); ?>">
              </div>
